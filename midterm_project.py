...
if __name__=='__main__':
	...
	st.write("Dashboard allows better decision making for targeted marketing. It is designed for \
		...
		of this dashboard is to hightlight this 20%")
	...
	are US, Canada, UK, European countries, etc. This information is vital for efficient \
	Ads spending on online store visitors of these regions only and ignoring rest of \
	the traffic.")
	...
	st.write("Following chart depicts proportion of mediums/devices/browsers/OS, etc which were used while making \
		purchase by customers. It shows clearly shows that Referral and Organic Search has generated \
		...
		approach is to be followed for rest of the choices which can be explored using top-left drop-down menu. ")
	...
	st.write("Following plots show high traffic mediums of access, browsers, whether store \
		is visited on mobile phone or not, etc. Higher the traffic, higher the odds of \
		finding potential customer therefore, budget and other efforts should be focused \
		on ways which generate high online traffic i.e Organic Search, Chrome browser, deskeopt \
		devices, etc")
	...
	st.write("Here interactive plot illustrates 80/20 rule, it is obvious that most of the online visits \
		...
		size of the circle represents the generated revenue which shows very few purchases generated millions of dollars \
		in revenue and it also highlights factors important in this process.")
	fig = px.scatter(
	    subset_df,
	    x="VisitorId",
	...
	st.write("As it's obvious by boxplot of revenue that most of the generated revenue is by outlier data points \
	...
	st.write("Here parallel plot highlights key factor in generating revenue which is total time spent \
		on online store by user, higher the time higher the odds of revenue. It suggests that we should \
